...
class ProductMapper:
    ...
    def _build_complete_mapping_table(self):
        """완전한 매핑 테이블 생성 (쉼표 있는/없는 버전 모두 포함)"""
        mapping = {}
...
if __name__ == "__main__":
    ...
    # 샘플 테스트 (쉼표 있는/없는 버전 모두 테스트)
    print("\n🧪 샘플 매핑 테스트:")
    test_cases = [
        ("서로 식혜", "2개, 1000ml"),  # 기존 (쉼표 있음)
        ("서로 식혜", "2개 1000ml"),   # 새로운 (쉼표 없음)
        ("[서로 단호박식혜] 수제 단호박 통째로", "4개, 1L"),  # 기존
        ("[서로 단호박식혜] 수제 단호박 통째로", "4개 1L"),   # 새로운
        ("[서로 진하고 깊은 식혜] 전통 국산 수제 식혜", "6개 1L"),  # 새로운
        ("없는제품", "없는옵션")  # 실패 케이스
    ]
